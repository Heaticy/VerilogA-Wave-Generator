...
class Signal:
    def __init__(self, name="signal", width=1, length=1, waves=[0]):
        ...
        self.wavelist_initial = self.get_wavelist_initial()
    ...
    def get_wavelist_declaration(self):
        ...
        return template.format(arg_1, arg_2)
    ...
    def get_wavelist_initial(self):
        ...
        if len(self.waves) <= 0:
            print("ERROR!!! The length of the wave list = 0 !!!")
            exit(0)
        elif len(self.waves) < self.length:
            print("ERROR!!! The length of the wave list > waves!!!")
            exit(0)
        for i in range(self.length):
            arg_2 = i
            arg_3 = self.waves[i]
            template_ = template_ + template.format(arg_1, arg_2, arg_3) + "\n"
...
class MultiSignal:
    ...
    def get_signals(self, args):
        thelist = []
        for arg in args:
            the_signal = Signal(name=arg[0], width=arg[1], length=arg[2], waves=arg[3])
    ...
    def get_wavelist_initial(self):
        result = ""
        for i in self.signals:
            result = result + i.wavelist_initial + "\n"
        return result
